...
- **No resume-after-pause.** `interpretFlow` always restarts from `trigger`; completed steps are not skipped. Re-running a `waiting` run re-executes every prior agent and writes duplicate `FlowRunStep` rows. Answer-and-continue for ask-user is structurally impossible today. → gates testing, error-repair, waiting-run monitoring.
  > **Partial fix (2026-07-10, Remediation WS-R2):** the top-level "answer-and-continue" resume race is closed — resume is now an atomic claim pinned to the run's graph snapshot (`src/features/flows/execute-flow.ts`).
  > **Loop-cursor fix (2026-07-10, Flows Parity Close-out):** the loop/parallel resume-from-cursor problem is now closed too — body-step outputs persist per-iteration (`nodeId#index`), so a mid-loop pause resumes without re-running prior iterations' side effects (`interpret.ts` `indexKey` threading; DB-verified no-re-run). Remaining follow-up: a loop that pauses MULTIPLE `humanReview` iterations at once with a single run-level reply routes it to the last-paused iteration — reply-routing, not a duplicate-side-effect issue; fix by carrying the paused-iteration key on the reply or per-iteration reply UI.

## Flows Parity Close-out (2026-07-10) — status update

Much of this roadmap shipped since the 2026-07-07 baseline (nested containers, retry/backoff/timeout, HTTP action, variables, all Data Operations, the typed datatree, resume + snapshot pinning, real triggers). The close-out addressed: **condition/switch inside a container** is now a loud validation error steering to `filter` (full branch-in-container remains a big bet); **webhook trigger vs HTTP action** disambiguated (distinct icon/color + one canonical name); **trigger-level filter condition** (gate a run before it starts); **loop resume-from-cursor** (above). **Canvas click-drag pan** also shipped separately. Remaining big bets (deliberately deferred): formula/expression mode, try-catch/rescue block, callable sub-flows, polling triggers with a cursor, lookup tables / cross-run state, Copilot diff-preview, multi-select, and full branch-in-container.
